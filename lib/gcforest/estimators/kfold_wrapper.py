...
class KFoldWrapper(object):
    ...
    def fit_transform(self, X, y, y_stratify, cache_dir=None, test_sets=None, eval_metrics=None, keep_model_in_mem=True):
        ...
        # Fit
        y_probas = []
        n_dims = X.shape[-1] # 784
        n_datas = X.size / n_dims
        inverse = False
        # train_idx:training set index; val_idx:validation set index
        for k in range(self.n_folds):
            ...
            if k == 0:
                if len(X.shape) == 2:
                    ...
                    # print(y_proba.shape[0]) # 12005
                    # print(y_proba.shape[1]) # 10
                    y_proba_cv = np.zeros((n_stratify, y_proba.shape[1]), dtype=np.float32) # 60000 x 10
                    # print(y_proba_cv.shape)
                else:
                    y_proba_cv = np.zeros((n_stratify, y_proba.shape[1], y_proba.shape[2]), dtype=np.float32)
                y_probas.append(y_proba_cv)
            # print(val_idx) # list of validation set indexs
            # print(y_proba) # numpy array
            # print(y_probas) # list: []
            # print(y_probas[0][val_idx, :]) # return the rows of the first element in y_probas from val_idx to the end.
        ...
        # print(y_probas[0].shape) # (60000, 10)
        # print(len(y_probas)) # 1
        return y_probas # [numpy.array object]
