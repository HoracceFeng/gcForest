...
class CascadeClassifier(object):
    ...
    def fit_transform(self, X_groups_train, y_train, X_groups_test, y_test, stop_by_test=False, train_config=None):
        ...
        if not type(X_groups_train) == list: # numpy.ndarray
        ...
        try:
            ...
            X_proba_train = np.zeros((n_trains, n_classes * self.n_estimators_1), dtype=np.float32) # (60000, 10*4)
            X_proba_test = np.zeros((n_tests, n_classes * self.n_estimators_1), dtype=np.float32) # (60000, 10*4)
            ...
            while 1:
                ...
                y_train_proba_li = np.zeros((n_trains, n_classes)) # 60000 x 10
                y_test_proba_li = np.zeros((n_tests, n_classes))
                for ei, est_config in enumerate(self.est_configs):
                    est = self._init_estimators(layer_id, ei) # class KFoldWrapper
                    # fit_transform
                    ...
                    y_probas = est.fit_transform(X_cur_train, y_train, y_train,
                            ...
                            keep_model_in_mem=train_config.keep_model_in_mem) # [(60000, 10)]
                    # train
                    X_proba_train[:, ei * n_classes: ei * n_classes + n_classes] = y_probas[0] # 60000 X (0 : 10) (10 : 20) (20 : 30) (30 : 40)
